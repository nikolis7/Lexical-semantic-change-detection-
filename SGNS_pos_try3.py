...
# Crreate a function for finding the common words either nn or vb
...
# Crreate a function for finding the common words  vb
...
############################ Found common noouns in the two dataset with mpore than 3 appereances in both datasets ############################
...
###################################################################################################
# Try pipeline 
sgns_pipeline(targets=targets, pretrained=False, alignment="OP", epochs=200, plot=True)
...
#################################################################################################
# Try pipeline 
sgns_pipeline(targets=targets, pretrained=False, alignment="incremental", epochs=100, plot=True)
#################################################################################################
...
def check_words_in_model(model, target_words):
    # Find words that are missing from the model vocabulary
    missing_words = [word for word in target_words if word not in model.wv]
    
    if missing_words:
        print(f"Missing words ({len(missing_words)}): {missing_words}")
    else:
        print("All target words are present in the model vocabulary.")
    
    return missing_words

# Usage example
model = Word2Vec.load("fine_tuned_incremental_model_1_epochs_100.model")
missing_words = check_words_in_model(model, targets)

model2 = Word2Vec.load("fine_tuned_model_1_epochs_100.model")
missing_words = check_words_in_model(model2, targets)
...
median_threshold = np.median(df['Cosine Similarity'])

median_threshold2 = np.median(df1['Cosine Similarity'])
...
def extract_align_and_filter_words(model1, model2, threshold, alignment="OP", count_n=3, top_n=300, pos_filter="both"):
    """
    ...
    filtered_common_words = [
        ...
        and len(word) > 3 and not any(char.isdigit() for char in word)
...
top_words, similarity_dict = extract_align_and_filter_words(model_1, model_2, threshold, alignment="Incre",count_n=10, top_n=100, pos_filter="nn")
...
def majority_vote_annotation(csv_files, output_file="final_annotation.csv"):
    """
    Create a final CSV file with words and majority-voted annotations from multiple annotation CSV files.
    ...
    required_columns = {'word', 'annotation'}
    for df in dfs:
        if not required_columns.issubset(df.columns):
            raise ValueError("Each CSV file must contain 'word' and 'annotation' columns.")
    
    # Step 2: Initialize a DataFrame for the final output (based on the first file)
    final_df = dfs[0][['word']].copy()  # Only retain the 'word' column for final output
...
final_df = majority_vote_annotation(csv_files, output_file="final_annotation.csv")
